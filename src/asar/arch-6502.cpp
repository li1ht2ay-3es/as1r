...
#define thefinal7(offset, len) as##len("STY", offset+0x80); as##len("STX", offset+0x82); \
...
#define onlythe8(left, right, offset) else if (match(left, right)) do { init_index(left, right); the8(offset, 1); end(); } while(0)
	...
	{
		blankinit();
		as0("PHP", 0x08); as0("ASL", 0x0A); as0("CLC", 0x18);
		as0("PLP", 0x28); as0("ROL", 0x2A);
		as0("SEC", 0x38);
		...
		as0("CLD", 0xD8);
		as0("INX", 0xE8); as0("NOP", 0xEA); as0("SED", 0xF8);
		as1("BRK", 0x00);
		end();
	}
	...
	else if (match("#", ""))
	{
		...
		as_rep("INX", 0xE8); as_rep("DEX", 0xCA);
